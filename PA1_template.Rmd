...
```
The mean of steps per day is `r step_mean` while the median is `r step_median`.
...
The total number of rows with NAs is `r incomplete_row`   
...
The mean of steps per day before filling missing values is `r step_mean`  and `r complete_step_mean` after filling. From this we can say there was no change in means.      
    
For median of steps per day before filling missing is`r step_median` while after is `r complete_step_median`. From this we can see that the median change to move towards the mean.       
